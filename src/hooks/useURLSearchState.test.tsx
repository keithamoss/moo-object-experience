...
import { createTestStore, AllTheProviders } from '../test-utils/test-helpers';
...
describe('useURLSearchState', () => {
  ...
  it('should dispatch setQuery when URL query changes', () => {
    ...
    renderHook(
      () => useURLSearchState(searchParams, '', ALL_SEARCHABLE_FIELD_NAMES),
      {
  ...
  it('should not dispatch when query unchanged', () => {
    ...
    renderHook(
      () => useURLSearchState(searchParams, 'stone', ALL_SEARCHABLE_FIELD_NAMES),
      {
        wrapper: ({ children }) => <AllTheProviders store={store}>{children}</AllTheProviders>,
      }
    );
  ...
  it('should handle empty query parameter', () => {
    ...
    renderHook(
      () => useURLSearchState(searchParams, 'existing', ALL_SEARCHABLE_FIELD_NAMES),
      {
        wrapper: ({ children }) => <AllTheProviders store={store}>{children}</AllTheProviders>,
      }
    );
  ...
  it('should use all fields when fields param is null', () => {
    ...
    renderHook(
      () => useURLSearchState(searchParams, '', []),
      {
        wrapper: ({ children }) => <AllTheProviders store={store}>{children}</AllTheProviders>,
      }
    );
  ...
  it('should not dispatch when fields are unchanged', () => {
    ...
    renderHook(
      () => useURLSearchState(searchParams, '', currentFields),
      {
        wrapper: ({ children }) => <AllTheProviders store={store}>{children}</AllTheProviders>,
      }
    );
    ...
    const setFieldsCalls = dispatchSpy.mock.calls.filter(
      call => call[0].type === setActiveFields.type
    );
